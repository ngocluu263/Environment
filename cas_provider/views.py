...
import cookielib # new!
import httpagentparser # for parsing browser
...
def login(request, template_name='cas/login.html',\
    ...
    if browser not in ALLOWED_BROWSERS:
	    return render_to_response('cas/bad_browser.html')
...
def logout(request, template_name='cas/logout.html',
    ...
    if request.user.is_authenticated():
        for ticket in ServiceTicket.objects.filter(user=request.user):
            ticket.delete()
        auth_logout(request)
    # new code! remove all domain cookies (including ASP ones)
    if url and auto_redirect:
       response = HttpResponseRedirect(url)
    else:
        response = render_to_response(template_name, {'url': url},context_instance=RequestContext(request))
    response.delete_cookie('ASP.NET_SessionId')
    response.delete_cookie('.ASPXAUTH')
    response.delete_cookie('.MONOAUTH')
    response.delete_cookie('.ASPXCASAUTH')
